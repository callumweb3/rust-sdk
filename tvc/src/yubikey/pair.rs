...
impl<D: DeviceOps + Send + 'static> Signer for YubiKeyPair<D> {
    fn sign(&self, message: &[u8]) -> SignerFuture<'_, anyhow::Result<Vec<u8>>> {
        ...
        Box::pin(async move {
            Self::device_op(devices, move |devices| devices.sign(serial, &pin, &message)).await
        })
    }
...
impl<D: DeviceOps + Send + 'static> Pair for YubiKeyPair<D> {
    fn decrypt(&self, ciphertext: &[u8]) -> SignerFuture<'_, anyhow::Result<Zeroizing<Vec<u8>>>> {
        ...
        Box::pin(async move {
            ...
            let shared_secret = Self::device_op(devices, move |devices| {
                devices.key_agreement(serial, &pin, &envelope.ephemeral_sender_public)
            })
            .await?;
...
impl Config {
    ...
    /// an unregistered serial, a missing or unprovisioned device, and a
    /// registry cache that no longer matches the device. The PIN prompt runs
    /// last, after the device checks, so nothing is typed at a device that
    /// cannot be used.
    pub(crate) async fn resolve_yubikey<D: DeviceOps + Send + 'static>(
        ...
        let public_key = {
            ...
            YubiKeyPair::device_op(devices, move |devices| {
                devices.verified_pair_public_key(serial)
            })
            .await
...
mod tests {
    ...
    #[tokio::test]
    async fn a_disconnected_device_is_reported_by_serial() {
        let device = provisioned_device();
        let absent = crate::config::turnkey::YubiKeySerial::from(0xdead_beef);
        let mut config = Config::default();
        config.register_yubikey(absent, device.operator_public_key());

        let error = config
            .resolve_yubikey(absent, device, fixed_pin())
            .await
            .unwrap_err();

        assert!(rendered(&error).contains("no connected YubiKey has serial deadbeef"));
    }
    ...
    async fn hardware_resolve_sign_and_decrypt_roundtrip() {
        ...
        let serial = *serial;
        let key = devices.verified_pair_public_key(serial).unwrap();
        let mut config = Config::default();
        config.register_yubikey(serial, key);

        let pin = PinAcquisition::Fixed(Pin::from(
            String::from_utf8(qos_client::yubikey::DEFAULT_PIN.to_vec()).unwrap(),
        ));
        let pair = config
            .resolve_yubikey(serial, PcscDevices, pin)
            .await
            .unwrap();
