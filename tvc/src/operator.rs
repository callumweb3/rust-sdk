...
impl Config {
    ...
    /// depend on an org being configured (pinned by
    ...
    pub(crate) async fn resolve_operator<D: DeviceOps + Send + 'static>(
        ...
        if let Some(hosted) = hosted {
            // Hosted operators sign through the API, so an offline
            // requirement is unsatisfiable; refuse before touching
            // credentials.
...
mod tests {
    ...
    async fn yubikey_default_resolves_a_device_backed_signer() {
        ...
        let operator = config
            .resolve_operator(device, None, None, SignerRequirement::Any, fixed_pin())
            .await
            .unwrap();
